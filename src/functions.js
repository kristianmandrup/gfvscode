...
function checkIfIsAGrailsProject() {
  if (getWorkspaceDir() != null) {
    if (fs.existsSync(path.join(getWorkspaceDir(), "grails-app"))) return true;
    else return false;
  } else {
    return null;
  }
...
function checkIfIsAGrailsPlugin() {
  ...
  if (getWorkspaceDir() != null) {
    if (
      fs.existsSync(
        path.join(getWorkspaceDir(), `${applicationName}GrailsPlugin.groovy`)
      )
    ) {
...
function defineAppProperties() {
  ...
  vscode.window.showInformationMessage(
    `Grails Version detected: ${grailsVersion}`
  );
}
...
function setStatusBarItem(status) {
  if (status != "dispose") {
    if (status == "create") stbarConfig = ["Creating Grails App...", "#ffffff"];
      ...
      stbarConfig = ["Performing Grails Application Init...", "#ffffff"];
    else if (status == "running")
      stbarConfig = ["Grails Application Running...", "#3bad3d"];
    else if (status == "stopping")
      stbarConfig = ["Stopping Application...", "#bd2438"];
    else if (status == "stopped")
      stbarConfig = ["Grails Application Stopped...", "#ffffff"];
...
function getFileListFolder(typeFiles) {
  ...
  directory = path.join(
    getWorkspaceDir(),
    `grails-app/${typeFiles}/${applicationName}`
  );
    ...
    result.stdout.on("data", (data) => {
      ...
      if (infoCatcher != null) {
        ...
        if (stopCatcher != null) {
          ...
          vscode.window.showInformationMessage(
            `Grails Application '${applicationName}' was stopped.`
          );
        }
      }
      resolve();
...
async function grailsRunCommand() {
  ...
  if (grailsCommand != null && grailsCommand.length > 0) {
    grailsChannel.show();
    let promise = new Promise((resolve) => {
      let result = cp.exec(`grails ${grailsCommand}`, {
        cwd: getWorkspaceDir(),
      });
      result.stdout.on("data", (data) => {
        outputFilter = data;
...
async function showDependencyReport() {
  await vscode.window
    .showInformationMessage(
      `Show Grails Dependency Report in Terminal or Save a Text File?`,
      "Terminal",
      "Text File"
    )
    .then((selection) => {
      if (selection === "Terminal") {
        grailsChannel.show();
        let promise = new Promise((resolve) => {
          let result = cp.exec(`grails dependency-report`, {
            cwd: getWorkspaceDir(),
          });
          result.stdout.on("data", (data) => {
        ...
        grailsChannel.show();
        grailsChannel.append(
          `Generating Text File of Grails Dependency Report...`
        );
        let promise = new Promise((resolve) => {
          let result = cp.exec(
            `grails dependency-report > dependency-report.txt`,
            { cwd: getWorkspaceDir() }
          );
          result.stdout.on("data", (data) => {
            grailsChannel.appendLine(`${data}`);
...
async function showHelp() {
  await vscode.window
    .showInformationMessage(
      `Show Grails Help in Terminal or Save a Text File?`,
      "Terminal",
      "Text File"
    )
    .then((selection) => {
      if (selection === "Terminal") {
        ...
        let promise = new Promise((resolve) => {
          let result = cp.exec(`grails help > grails-help.txt`, {
            cwd: getWorkspaceDir(),
          });
          result.stdout.on("data", (data) => {
            grailsChannel.appendLine(`${data}`);
...
async function listPlugins() {
  await vscode.window
    .showInformationMessage(
      `Show Grails List Plugins in Terminal or Save a Text File?`,
      "Terminal",
      "Text File"
    )
    .then((selection) => {
      if (selection === "Terminal") {
        grailsChannel.show();
        let promise = new Promise((resolve) => {
          let result = cp.exec(`grails list-plugins`, {
            cwd: getWorkspaceDir(),
          });
          result.stdout.on("data", (data) => {
            grailsChannel.appendLine(`${data}`);
        ...
        let promise = new Promise((resolve) => {
          let result = cp.exec(
            `grails list-plugins > grails-list-plugins.txt`,
            { cwd: getWorkspaceDir() }
          );
          result.stdout.on("data", (data) => {
            grailsChannel.appendLine(`${data}`);
...
async function showStats() {
  await vscode.window
    .showInformationMessage(
      `Show Grails Stats in Terminal or Save a Text File?`,
      "Terminal",
      "Text File"
    )
    .then((selection) => {
      if (selection === "Terminal") {
        ...
        let promise = new Promise((resolve) => {
          let result = cp.exec(`grails stats > grails-stats.txt`, {
            cwd: getWorkspaceDir(),
          });
          result.stdout.on("data", (data) => {
            grailsChannel.appendLine(`${data}`);
...
function generateDoc() {
  ...
  vscode.window.showInformationMessage(
    `Generating documentation os project...`
  );
  grailsChannel.show();
  let promise = new Promise((resolve) => {
...
function packagePlugin() {
  if (isAPluginGrails != "yes") {
    vscode.window.showErrorMessage(
      `Unable to create a package if the project is not a Grails Plugin!`
    );
  } else {
    grailsChannel.show();
    let promise = new Promise((resolve) => {
...
function mavenInstall() {
  if (isAPluginGrails != "yes") {
    vscode.window.showErrorMessage(
      `Unable to perform Maven Install if the project is not a Grails Plugin!`
    );
  } else {
    grailsChannel.show();
    let promise = new Promise((resolve) => {
...
async function addProxy(proxyWithUser) {
  if (!filterVersion(getGrailsVersion(), "add-proxy")) {
      ...
      if (hostProxy != null && hostProxy.length > 0) {
        ...
        if (portProxy != null && portProxy.length > 0) {
          if (!isNaN(portProxy)) {
            if (proxyWithUser) {
              do
                usernameProxy = await vscode.window.showInputBox(
                  cts.optInputUsernameProxy
                );
              while (usernameProxy == null && usernameProxy.length <= 0);

              do
                passwordProxy = await vscode.window.showInputBox(
                  cts.optInputPasswordProxy
                );
              while (passwordProxy == null && passwordProxy.length <= 0);
            }
          } else {
            vscode.window.showErrorMessage(
              `The port of proxy configuration is not a number.`
            );
            return;
          }
        } else {
          vscode.window.showErrorMessage(
            `The port of proxy configuration required.`
          );
          return;
        }
      } else {
        vscode.window.showErrorMessage(
          `The hostname of proxy configuration required.`
        );
        return;
      }
    } else {
      vscode.window.showErrorMessage(
        `The name of proxy configuration required.`
      );
      return;
    }
    ...
    if (proxyWithUser) {
      proxyCommand = proxyCommand.concat(
        ` --username=${usernameProxy} --password=${passwordProxy}`
      );
    }
...
async function removeProxy() {
  if (!filterVersion(getGrailsVersion(), "remove-proxy")) {
    ...
    let quickPick = await vscode.window.showQuickPick(
      proxyNamesList.toString().split(","),
      {
    ...
    if (quickPick != null && quickPick.length > 0) {
      if (quickPick != "currentProxy") {
        ...
        let promise = new Promise((resolve) => {
          vscode.window.showInformationMessage(
            `Remove '${quickPick}' alias configuration proxy.`
          );
          let result = cp.exec(`grails remove-proxy ${quickPick}`, {
            cwd: getWorkspaceDir(),
          });
          result.stdout.on("data", (data) => {
            outputFilter = data;
        ...
        vscode.window.showWarningMessage(
          `To remove de currentProxy configuration, go to ProxySettings.groovy and delete the line of file and run this command again.`
        );
      }
    }
  }
...
async function setProxy() {
  if (!filterVersion(getGrailsVersion(), "set-proxy")) {
    ...
    if (quickPick != null && quickPick.length > 0) {
      ...
      let promise = new Promise((resolve) => {
        vscode.window.showInformationMessage(
          `Set alias configuration proxy to '${quickPick}'.`
        );
        let result = cp.exec(`grails set-proxy ${quickPick}`, {
          cwd: getWorkspaceDir(),
        });
        result.stdout.on("data", (data) => {
          outputFilter = data;
      ...
      vscode.window.showInputBox(cts.optInputAppName).then((appName) => {
        if (appName != null && appName.length > 0) {
          ...
          let promise = new Promise((resolve) => {
            ...
            let result = cp.exec(`grails create-app ${appName}`, {
              cwd: folder[0].fsPath,
            });
            result.stdout.on("data", (data) => {
              outputFilter = data;
              infoCatcher = outputFilter.match(/\w.+/gi);
              createCatcher = outputFilter.match(
                /Created Grails Application/gi
              );
              if (infoCatcher != null) {
                grailsChannel.appendLine(`${infoCatcher[0]}`);
      ...
      vscode.window.showInputBox(cts.optInputPluginName).then((pluginName) => {
        if (pluginName != null && pluginName.length > 0) {
          ...
          let promise = new Promise((resolve) => {
            ...
            let result = cp.exec(`grails create-plugin ${pluginName}`, {
              cwd: folder[0].fsPath,
            });
            result.stdout.on("data", (data) => {
              outputFilter = data;
...
function createDomainClass() {
  vscode.window.showInputBox(cts.optInputDomainName).then((domainName) => {
    if (domainName != null && domainName.length > 0) {
      ...
      let promise = new Promise((resolve) => {
        vscode.window.showInformationMessage(
          `Creating '${domainName}' Domain Class...`
        );
        let result = cp.exec(`grails create-domain-class ${domainName}`, {
          cwd: getWorkspaceDir(),
        });
        result.stdout.on("data", (data) => {
          outputFilter = data;
          ...
          if (infoCatcher != null) {
            ...
            if (createCatcher != null) {
              vscode.window.showInformationMessage(
                `Domain Class '${domainName}' was created.`
              );
            }
          }
          resolve();
...
function createController() {
  vscode.window
    .showInputBox(cts.optInputControllerName)
    .then((controllerName) => {
      if (controllerName != null && controllerName.length > 0) {
        grailsChannel.show();
        let promise = new Promise((resolve) => {
          vscode.window.showInformationMessage(
            `Creating '${controllerName}' Controller...`
          );
          let result = cp.exec(`grails create-controller ${controllerName}`, {
            cwd: getWorkspaceDir(),
          });
          result.stdout.on("data", (data) => {
            outputFilter = data;
            infoCatcher = outputFilter.match(/\w.+/gi);
            createCatcher = outputFilter.match(
              /Created file grails-app\/controllers/gi
            );
            if (infoCatcher != null) {
              grailsChannel.appendLine(`${infoCatcher[0]}`);
              if (createCatcher != null) {
                vscode.window.showInformationMessage(
                  `Controller '${controllerName}Controller' was created.`
                );
              }
            }
            resolve();
...
function createService() {
  vscode.window.showInputBox(cts.optInputServiceName).then((serviceName) => {
    if (serviceName != null && serviceName.length > 0) {
      ...
      let promise = new Promise((resolve) => {
        vscode.window.showInformationMessage(
          `Creating '${serviceName}' Service...`
        );
        let result = cp.exec(`grails create-service ${serviceName}`, {
          cwd: getWorkspaceDir(),
        });
        result.stdout.on("data", (data) => {
          outputFilter = data;
          infoCatcher = outputFilter.match(/\w.+/gi);
          createCatcher = outputFilter.match(
            /Created file grails-app\/services/gi
          );
          if (infoCatcher != null) {
            grailsChannel.appendLine(`${infoCatcher[0]}`);
            if (createCatcher != null) {
              vscode.window.showInformationMessage(
                `Service '${serviceName}Service' was created.`
              );
            }
          }
          resolve();
...
function createFilter() {
  if (!filterVersion(getGrailsVersion(), "create-filter")) {
    vscode.window.showErrorMessage(
      `The 'create-filter' method is deprecated. Use 'create-interceptior'.`
    );
  } else {
    vscode.window.showInputBox(cts.optInputFilterName).then((filterName) => {
      if (filterName != null && filterName.length > 0) {
        ...
        let promise = new Promise((resolve) => {
          vscode.window.showInformationMessage(
            `Creating '${filterName}' Filter...`
          );
          let result = cp.exec(`grails create-filter ${filterName}`, {
            cwd: getWorkspaceDir(),
          });
          result.stdout.on("data", (data) => {
            outputFilter = data;
            infoCatcher = outputFilter.match(/\w.+/gi);
            createCatcher = outputFilter.match(
              /Created file grails-app\/conf/gi
            );
            if (infoCatcher != null) {
              grailsChannel.appendLine(`${infoCatcher[0]}`);
              if (createCatcher != null) {
                vscode.window.showInformationMessage(
                  `Filter '${filterName}' was created.`
                );
              }
            }
            resolve();
...
function createInterceptor() {
  if (!filterVersion(getGrailsVersion(), "create-interceptor")) {
    vscode.window.showErrorMessage(
      `The 'create-interceptor' method not supported in Grails Version < 3. Use 'create-filters'.`
    );
  } else {
    vscode.window
      .showInputBox(cts.optInputInterceptorName)
      .then((interceptorName) => {
        if (interceptorName != null && interceptorName.length > 0) {
          grailsChannel.show();
          let promise = new Promise((resolve) => {
            vscode.window.showInformationMessage(
              `Creating '${interceptorName}' Interceptor...`
            );
            let result = cp.exec(
              `grails create-interceptor ${interceptorName}`,
              { cwd: getWorkspaceDir() }
            );
            result.stdout.on("data", (data) => {
              outputFilter = data;
              ...
              if (infoCatcher != null) {
                ...
                if (createCatcher != null) {
                  vscode.window.showInformationMessage(
                    `Interceptor '${interceptorName}' was created.`
                  );
                }
              }
              resolve();
...
function createHibernateCfgXML() {
  ...
  if (fs.existsSync(hibernateFile)) {
    vscode.window
      .showWarningMessage(
        `The file 'hibernate.cfg.xml' already exists. Overwrite it?`,
        "Yes",
        "No"
      )
      .then((selection) => {
        if (selection === "Yes") {
          try {
            ...
            grailsChannel.show();
            let promise = new Promise((resolve) => {
              let result = cp.exec(`grails create-hibernate-cfg-xml`, {
                cwd: getWorkspaceDir(),
              });
              result.stdout.on("data", (data) => {
                outputFilter = data;
                infoCatcher = outputFilter.match(/\w.+/gi);
                createCatcher = outputFilter.match(
                  /Created file grails-app\\conf\\hibernate\\hibernate.cfg.xml/gi
                );
                if (infoCatcher != null) {
                  grailsChannel.appendLine(`${infoCatcher[0]}`);
                  if (createCatcher != null) {
                    vscode.window.showInformationMessage(
                      `The 'hibernate.cfg.xml' file was created.`
                    );
                  }
                }
                resolve();
...
function createScript() {
  vscode.window.showInputBox(cts.optInputScriptName).then((scriptName) => {
    if (scriptName != null && scriptName.length > 0) {
      ...
      let promise = new Promise((resolve) => {
        vscode.window.showInformationMessage(
          `Creating '${scriptName}' Script...`
        );
        let result = cp.exec(`grails create-script ${scriptName}`, {
          cwd: getWorkspaceDir(),
        });
        result.stdout.on("data", (data) => {
          outputFilter = data;
          ...
          if (infoCatcher != null) {
            ...
            if (createCatcher != null) {
              vscode.window.showInformationMessage(
                `Script '${scriptName}.groovy' was created.`
              );
            }
          }
          resolve();
      ...
      let promise = new Promise((resolve) => {
        ...
        result.stdout.on("data", (data) => {
          ...
          if (infoCatcher != null) {
            ...
            if (createCatcher != null) {
              vscode.window.showInformationMessage(
                `Taglib '${tagLibName}TagLib.groovy' was created.`
              );
            }
          }
          resolve();
...
function createUnitTest() {
  vscode.window.showInputBox(cts.optInputUnitTestName).then((unitTestName) => {
    if (unitTestName != null && unitTestName.length > 0) {
      ...
      let promise = new Promise((resolve) => {
        vscode.window.showInformationMessage(
          `Creating '${unitTestName}' Unit Test...`
        );
        let result = cp.exec(`grails create-unit-test ${unitTestName}`, {
          cwd: getWorkspaceDir(),
        });
        result.stdout.on("data", (data) => {
          outputFilter = data;
          ...
          if (infoCatcher != null) {
            ...
            if (createCatcher != null) {
              vscode.window.showInformationMessage(
                `Unit Test '${unitTestName}Spec.groovy' was created.`
              );
            }
          }
          resolve();
...
function createIntegrationTest() {
  vscode.window
    .showInputBox(cts.optInputIntegrationTestName)
    .then((integrationTestName) => {
      if (integrationTestName != null && integrationTestName.length > 0) {
        grailsChannel.show();
        let promise = new Promise((resolve) => {
          vscode.window.showInformationMessage(
            `Creating '${integrationTestName}' Integration Test...`
          );
          let result = cp.exec(
            `grails create-integration-test ${integrationTestName}`,
            { cwd: getWorkspaceDir() }
          );
          result.stdout.on("data", (data) => {
            outputFilter = data;
            ...
            if (infoCatcher != null) {
              ...
              if (createCatcher != null) {
                vscode.window.showInformationMessage(
                  `Integration Test '${integrationTestName}IntegrationSpec.groovy' was created.`
                );
              }
            }
            resolve();
  ...
  if (!filterVersion(getGrailsVersion(), "create-pom")) {
    ...
    vscode.window.showInputBox(cts.optInputPomGroupId).then((groupIdName) => {
      if (groupIdName != null && groupIdName.length > 0) {
        ...
        let promise = new Promise((resolve) => {
          vscode.window.showInformationMessage(
            `Creating POM.xml with '${groupIdName}' group id...`
          );
          let result = cp.exec(`grails create-pom ${groupIdName}`, {
            cwd: getWorkspaceDir(),
          });
          result.stdout.on("data", (data) => {
            outputFilter = data;
  ...
  vscode.window.showInputBox(cts.optInputGenerateViews).then((generateName) => {
    if (generateName != null && generateName.length > 0) {
      ...
      let promise = new Promise((resolve) => {
        ...
        let result = cp.exec(`grails generate-views ${generateName}`, {
          cwd: getWorkspaceDir(),
        });
        result.stdout.on("data", (data) => {
          outputFilter = data;
...
function generateControllers() {
  vscode.window
    .showInputBox(cts.optInputGenerateControllers)
    .then((generateName) => {
      if (generateName != null && generateName.length > 0) {
        grailsChannel.show();
        let promise = new Promise((resolve) => {
          vscode.window.showInformationMessage(`Generate Controllers...`);
          let result = cp.exec(`grails generate-controller ${generateName}`, {
            cwd: getWorkspaceDir(),
          });
          result.stdout.on("data", (data) => {
            outputFilter = data;
  ...
  vscode.window.showInputBox(cts.optInputGenerateAll).then((generateName) => {
    if (generateName != null && generateName.length > 0) {
      ...
      let promise = new Promise((resolve) => {
        ...
        let result = cp.exec(`grails generate-all ${generateName}`, {
          cwd: getWorkspaceDir(),
        });
        result.stdout.on("data", (data) => {
          outputFilter = data;
